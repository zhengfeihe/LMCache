...
| `MP_STORE_END` | `device`, `stored_count`, `engine_id`, `model_name`, `total_bytes`, `num_tokens`, `reserve_seconds`, `objects_total`, `objects_skipped`, `longest_skip_free_run` | `str`, `int`, `int`, `str`, `int`, `int`, `float`, `int`, `int`, `int` |
...
`objects_total` / `objects_skipped` / `longest_skip_free_run` summarize the
storage-manager skips (`None` entries) across all object groups; the longest
skip-free run bounds the batch size a batched store could actually use.
