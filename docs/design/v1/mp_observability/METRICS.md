...
| `lmcache_mp.l0_l1_store_skipped_ratio` | `lmcache_mp_l0_l1_store_skipped_ratio` | Histogram | `MP_STORE_END` | `objects_skipped / objects_total` per request |
| `lmcache_mp.l0_l1_store_longest_skip_free_run` | `lmcache_mp_l0_l1_store_longest_skip_free_run` | Histogram | `MP_STORE_END` | `longest_skip_free_run` metadata per request |
...
with CUDA event pairs (disable with `--disable-phase-timing`; recording
also follows `--disable-observability` / `--disable-metrics`, since only
a metrics subscriber consumes the samples); the store/retrieve handlers
drain finished pairs and publish them as `MP_TRANSFER_PHASE_SAMPLES`.
